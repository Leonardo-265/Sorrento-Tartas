...
Abrí **`js/config.js`** para cambiar:
...
No hace falta tocar `js/app.js` para estos cambios.

## Cómo funciona

1. El catálogo de tartas se carga en vivo desde la tabla `productos` de Supabase (categoría "Tartas", subcategorías Tarta Grande / Media Tarta / Tartelleta). Si falla la conexión, usa una copia de respaldo embebida en `js/app.js` (`PRODUCTOS_FALLBACK`).
